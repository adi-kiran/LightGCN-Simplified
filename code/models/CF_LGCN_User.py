...
class CF_LGCN_U(torch.nn.Module):
    def __init__(self, num_users, embedding_dim=64, num_layers=3, fusion_type="mean", alpha=None, **kwargs):
        super().__init__()
        ...
        self.user_embedding = torch.nn.Embedding(num_embeddings = num_users, embedding_dim = self.embedding_dim)
    ...
    def get_embedding(self, edge_index, rev_edge_index):
        ...
        assert self.num_users == num_users
        u_x = self.user_embedding.weight # Layer 0 = User embedding
        i_x = torch.zeros([num_items,self.embedding_dim]) # infer num_books from edge index to make it inductive
        torch.nn.init.xavier_uniform_(i_x)
        out = u_x * self.alpha[0] if self.fusion_type=="mean" else u_x
        user_embeddings.append(out)
        for i in range(1,self.num_layers+1):
            if i%2==1:
                # Odd numbered Layer = UE-GCN Layer, i.e., User Embedding used to calculate Item Embedding
                # x now holds user embedding, so we pass R^T (rev_edge_index, ie, items->users) and it will calculate the item embedding
                ...
                # Even numbered Layer = EU-GCN Layer, i.e., Item Embedding used to calculate User Embedding
                # x now holds item embedding so we pass R (edge_index, ie, users->items) and it will calculate the user embedding
    ...
    def forward(self, edge_index, rev_edge_index):
        ...
        return { "user_final_embedding": user_final_embedding, 
                 ...
                 "user_initial_embedding": self.user_embedding.weight }
